...
<template>
      ...
      <q-card-section class="no-padding">
        ...
        >
          ...
          <div
            ...
            "
          >
            <!-- 对特殊VIP限额、特殊单注单场限额类型用户的限额需求，需联系B端风控 -->
            <a-tooltip trigger="hover">
              <template slot="title">
                {{ $t("external.betting_.label2") }}
              </template>
              <a-icon type="question-circle" class=" fs15" />
            </a-tooltip>
            <!-- 用户投注限额是正常用户的 -->
            {{ $t("external.betting_.label1") }}
            <a-select
              v-model="percentageLimit"
              style="width:105px;margin-top:7px"
              class="disabled-white"
            >
            <!-- :disabled="[3, 4].includes(detailObj.specialBettingLimitType)"  改动 53行-->
              <a-select-option
 ...
 <!-- 改动 135行 && -->
        <div
          v-if="can_edit && [1, 2].includes(detailObj.specialBettingLimitType)"
...
export default {
  ...
  data() {
    return {
      ...
      specialBettingLimit_title: i18n.t(
        "external.merchant.bettinguser.index.specialBettingLimit_title"
      ),
  ...
  methods: {
    ...
    handle_post_userReport_updateRcsLimit() {
      ...
      let params = {        // 改动 307-316
        rcsUserConfigVo: {
          userId: this.detailObj.userId,
          specialBettingLimit: 2,
          percentage: percentageLimit*1,
          // betExtraDelay:'',
          // marketLevel:'',
          remarks: this.specialBettingLimitRemark
        }
      };
